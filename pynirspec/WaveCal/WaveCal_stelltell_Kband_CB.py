...
Aug. 2017 edited to have telluric/stellar sliders - CB
"""
...
#            if posneg == 1 : baseline_neg = coeff[4] + coeff[3]*np.arange(1024) + coeff[2]*np.arange(1024)**2 + coeff[1]*np.arange(1024)**3 + coeff[0]*np.arange(1024)**4# + coeff[0]*np.arange(1024)**3
		...
		for self.sec in secs:
			...
			self.modflux, newfl_obs = self.comparemodelanddata(self.WaveMod, self.ModSpec, self.WaveObsPN[0], self.SkyObsPN[0])
					...
					while(True):
						try:
							...
							if self.sec == 'full':
								...
								self.PlotModObs(self.WaveMod, self.modflux, self.WaveObsPN[0], newfl_obs, self.ObsLabelPN[0])
			
							else:
								if len(self.modflux) == self.modellength:
									wvm, fm = self.Breakintofour_better(self.WaveMod, self.modflux, section=self.sec)
									pixels_quarter = np.arange(256)			### 1024/4
								else:
									wvm = self.Breakintofour_better(self.WaveMod, section=self.sec)
									fm = self.modflux.copy()
									pixels_quarter = np.arange(256)
									
								fo = self.Breakintofour_better(newfl_obs,section=self.sec)
								wvo = self.DSFunc(pixels_quarter, self.Coefs[0])
					...
					while(True):
						try:
							...
							if self.sec == 'full':
								...
								self.PlotModObs(self.WaveMod, self.modflux, self.WaveObsPN[1], newfl_obs, self.ObsLabelPN[1])
							else:
								if len(self.modflux) == self.modellength:
									wvm, fm = self.Breakintofour_better(self.WaveMod, self.modflux, section=self.sec)
									pixels_quarter = np.arange(256)
									...
									pixels_quarter = np.arange(256)
								fo = self.Breakintofour_better(newfl_obs,section=self.sec)
								wvo = self.DSFunc(pixels_quarter, self.Coefs[1])
					...
					for i in range(0,len(self.Coefs)):
						if (self.GoodFit[i] != True):
							...
							if self.sec == 'full':
								...
								self.Coefs[i], self.WaveObsPN[i] = self.FitDS(self.WaveMod, self.modflux, newfl_obs, self.Coefs[i])                                   
								self.PlotModObs(self.WaveMod, self.modflux, self.WaveObsPN[i], newfl_obs, self.ObsLabelPN[i])
							else:
								if len(self.modflux) == self.modellength:
									wvm, fm = self.Breakintofour_better(self.WaveMod, self.modflux, section=self.sec)
								else:
								...
								if len(self.WaveObsPN[i]) == 1024:
									wvo, fo = self.Breakintofour_better(self.WaveObsPN[i], newfl_obs,section=self.sec)
								else:
									fo = self.Breakintofour_better(newfl_obs,section=self.sec)
	...
	def PlotModObs(self, wave_mod, flux_mod, wave_obs, flux_obs, label, block=False):
		...
		plt.plot(wave_obs, flux_obs, color='red')
		x1,x2,y1,y2 = plt.axis()
		plt.axis((x1, x2, np.nanmin([np.nanmin(flux_mod), np.nanmin(flux_obs)])-0.15, np.nanmax([np.amax(flux_mod),1.0])))
		plt.title(label)		
	...
	def FitDS(self, WLmod, Fmod, Fobs, CoefsInit):
		...
		params = Parameters()
		params.add('A',value=CoefsInit[0], vary=True,min=CoefsInit[0]-0.05,max=CoefsInit[0]+0.05)
...
#        params.add('F',value=1e-6, vary=True)
		...
		result = minimize(self.residual, params, args=(Fobs, WLmod, Fmod), method='least-sq') #'least-sq')
