...
class Spec1D():
	...
	def extract(self,PSF,method='single'):
		...
		for i in np.arange(sh):
			...
			flux_pos[i] = (PSF[:int(npsf/2)-1]*im[:int(npsf/2)-1,i]/uim[:int(npsf/2)-1,i]**2).sum() / (PSF[:int(npsf/2)-1]**2/uim[:int(npsf/2)-1,i]**2).sum()
			flux_neg[i] = (PSF[int(npsf/2):-1]*im[int(npsf/2):-1,i]/uim[int(npsf/2):-1,i]**2).sum() / (PSF[int(npsf/2):-1]**2/uim[int(npsf/2):-1,i]**2).sum()
			#flux_pos[i] = (PSF[:npsf/2-1]*im[:npsf/2-1,i]).sum() / (PSF[:npsf/2-1]**2).sum()
			#flux_neg[i] = (PSF[npsf/2:-1]*im[npsf/2:-1,i]).sum() / (PSF[npsf/2:-1]**2).sum()

			# Error in fluxes
			uflux_pos[i] = np.sqrt(1.0/(PSF[:int(npsf/2)-1]**2/uim[:int(npsf/2)-1,i]**2.).sum())
			uflux_neg[i] = np.sqrt(1.0/(PSF[int(npsf/2):-1]**2/uim[int(npsf/2):-1,i]**2.).sum())

			# Same as above for the sky, but variance is zero (sky assumed to be noise-free)
			sky_pos[i] = (PSF[:int(npsf/2)-1]*sky[:int(npsf/2)-1,i]).sum() / (PSF[:int(npsf/2)-1]**2).sum()
			sky_neg[i] = -(PSF[int(npsf/2):-1]*sky[int(npsf/2):-1,i]).sum() / (PSF[int(npsf/2):-1]**2).sum()
			usky_pos[i] = np.sqrt(1.0/(PSF[:int(npsf/2)-1]**2/usky[:int(npsf/2)-1,i]**2.).sum())
			usky_neg[i] = np.sqrt(1.0/(PSF[int(npsf/2):-1]**2/usky[int(npsf/2):-1,i]**2.).sum())
